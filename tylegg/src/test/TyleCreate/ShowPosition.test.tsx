...
const Template = () => {
  ...
  return (
    <ShowPosition positions={positions} tyleInfoSetFun={setSelectedInfoFunc} operatorImgData={operatorImgData} />
  )
}

describe("ShowPosition의 데이터 확인 테스트", () => {
  test("포지션 목록들이 재대로 갔는지", async () => {
    render(
      <Template />
    );
    const duelist = await screen.findByTestId("duelist");
    const initiator = await screen.findByTestId("initiator");
    const sentinel = await screen.findByTestId("sentinel");
    const controller = await screen.findByTestId("controller");
    
    expect(duelist).toHaveTextContent("타격대");
    expect(initiator).toHaveTextContent("척후대");
    expect(sentinel).toHaveTextContent("감시자");
    expect(controller).toHaveTextContent("전략가");
  });
})